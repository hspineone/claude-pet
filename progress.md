...
- [x] `gradle/libs.versions.toml` 에 `net.java.dev.jna:jna-platform` 5.18.1 추가
- [x] `composeApp/build.gradle.kts` 에 `implementation(libs.jna.platform)` 추가
- [x] `data/platform/ForegroundAppProbe.kt` 를 `os.name` 기반 분기 구조로 리팩토링
  - macOS: 현재 `osascript` 경로 유지
  - Windows: JNA 기반 신규 구현 (`GetForegroundWindow` → `GetWindowThreadProcessId` → `OpenProcess` + `QueryFullProcessImageName`)
  - 기타: null
- [x] 실패 시 조용히 null 반환 + `java.util.logging` FINE 레벨 로깅
- [ ] nativeDistributions 모듈 설정 검토 — **확인 필요** (packageDmg / packageMsi 번들 실행 전까지 jlink 영향 미확인)
- [x] 빌드 검증: macOS `./gradlew :composeApp:compileKotlinJvm` 성공
- [x] 실행 검증: macOS `./gradlew :composeApp:run` 기동 성공 (MainKt PID 감지, JNA 경로 미진입)
